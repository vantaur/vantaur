...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVantaurAmount"))
        settings.setValue("nAnonymizeVantaurAmount", 1000);
    nAnonymizeVantaurAmount = settings.value("nAnonymizeVantaurAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeVantaurAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeVantaurAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeVantaurAmount:
            return QVariant(nAnonymizeVantaurAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeVantaurAmount:
            nAnonymizeVantaurAmount = value.toInt();
            settings.setValue("nAnonymizeVantaurAmount", nAnonymizeVantaurAmount);
            emit AnonymizeVantaurAmountChanged(nAnonymizeVantaurAmount);
            break;
        default:
            break;
        }
