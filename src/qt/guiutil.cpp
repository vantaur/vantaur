...
{
    // Convert Vantaur:// to Vantaur:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Vantaur://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Vantaur:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Vantaur.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Vantaur.lnk
...
{
    return GetAutostartDir() / "Vantaur.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Vantaur.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Vantaur\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Vantaur app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Vantaur app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Vantaur-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Vantaur-qt [" + tr("command-line options") + "]                     " + "\n";
