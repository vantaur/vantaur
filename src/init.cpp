...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Vantaur.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: vantaurd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizevantauramount=<n> " + _("Keep N Vantaur anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Vantaur is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vantaur is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Vantaur version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Vantaur server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Vantaur") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Vantaur to complete") << "\n";
    ...
    nAnonymizeVantaurAmount = GetArg("-anonymizevantauramount", 0);
    if(nAnonymizeVantaurAmount > 999999) nAnonymizeVantaurAmount = 999999;
    if(nAnonymizeVantaurAmount < 2) nAnonymizeVantaurAmount = 2;
    ...
    LogPrintf("Anonymize Vantaur Amount %d\n", nAnonymizeVantaurAmount);
